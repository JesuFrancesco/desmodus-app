...
from app.schemas.archivos import ArchivosCreate


def crear_archivo(session: Session, archivo_data: ArchivosCreate):
    archivo = Archivo(**archivo_data.model_dump(exclude={"id"}))
    session.add(archivo)
    session.commit()
...
def get_all_archivos(session: Session):
    archivos = session.exec(select(Archivo)).all()
    return archivos
...
def update_one_archivo(session: Session, archivo: ArchivosCreate, archivo_id: int):
    archivo_db = session.get(Archivo, archivo_id)
